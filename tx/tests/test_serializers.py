...
def event(financial_year):
    return Event.objects.create(
        date=date(2023, 6, 1), description="Test event", financial_year=financial_year
    )
...
def test_financial_year_end_date_before_start_date():
    invalid_data = {"start_date": "2023-12-31", "end_date": "2023-01-01"}
...
def test_financial_year_same_start_and_end_date():
    same_date_data = {"start_date": "2023-01-01", "end_date": "2023-01-01"}
...
def test_financial_year_serialization(financial_year, api_request):
    serializer = FinancialYearSerializer(
        financial_year, context={"request": api_request}
    )
...
def test_transaction_serialization(account, event):
    transaction = Transaction.objects.create(
        amount=Decimal("100.50"), account=account, direction="credit", event=event
    )
...
@pytest.mark.django_db
def test_event_create_with_balanced_transactions(
    cash_account, revenue_account, financial_year
):
    data = {
        "date": "2023-06-01",
...
@pytest.mark.django_db
def test_event_create_with_unbalanced_transactions(
    cash_account, revenue_account, financial_year
):
    data = {
        "date": "2023-06-01",
        ...
        "transactions": [
            {"amount": "100.00", "account": cash_account.id, "direction": "debit"},
            {"amount": "50.00", "account": revenue_account.id, "direction": "credit"},
        ],
    }
    serializer = EventSerializer(data=data)
    assert not serializer.is_valid()
...
@pytest.mark.django_db
def test_event_create_multiple_transactions_balanced(
    cash_account, revenue_account, financial_year
):
    ...
    data = {
        ...
        "financial_year": financial_year.id,
        "transactions": [
            {"amount": "150.00", "account": cash_account.id, "direction": "debit"},
            {"amount": "50.00", "account": expense_account.id, "direction": "debit"},
            {"amount": "200.00", "account": revenue_account.id, "direction": "credit"},
        ],
    }
    serializer = EventSerializer(data=data)
    assert serializer.is_valid()
...
def test_event_serialization(
    cash_account, revenue_account, financial_year, api_request
):
    event = Event.objects.create(
        date=date(2023, 6, 1), description="Test event", financial_year=financial_year
    )
    Transaction.objects.create(
        amount=Decimal("100.00"), account=cash_account, direction="debit", event=event
    )
    Transaction.objects.create(
