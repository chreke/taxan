...
def test_create_event(api_client, financial_year, account):
    ...
    data = {
        ...
        "transactions": [
            {"amount": "100.00", "account": account.id, "direction": "debit"},
            {"amount": "100.00", "account": account.id, "direction": "credit"},
        ],
    }
...
def test_list_events(api_client, financial_year):
    Event.objects.create(
        date="2023-06-15", description="Test Event 1", financial_year=financial_year
    )
    Event.objects.create(
        date="2023-06-16", description="Test Event 2", financial_year=financial_year
    )
    url = reverse("event-list")
    response = api_client.get(url)
    assert response.status_code == status.HTTP_200_OK
